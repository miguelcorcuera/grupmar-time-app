-- ============================================================================
-- GrupMar Time v16.10.4 - FIX company_id obligatorio en departments/work_centers
--
-- Error:
-- null value in column "company_id" of relation "departments" violates not-null constraint
--
-- Causa:
-- departments/work_centers tienen company_id NOT NULL.
-- El formulario enviaba company_name pero no company_id.
--
-- Solución:
-- RPC resuelve company_id desde company_id o company_name.
-- También mantiene código automático sin pedirlo en formulario.
-- ============================================================================
...
create or replace function public.gmt_resolve_company_id(p_company_id uuid, p_company_name text)
returns uuid
language plpgsql
security definer
set search_path = public
...
begin
  ...
  execute $q$
    ...
    limit 1
  $q$
  using v_company_name
  into v_company_id;

  -- Si no existe Grupo Marport/empresa elegida, crearla de forma mínima.
  if v_company_id is null then
    if public.gmt_table_has_column('companies', 'name') then
      v_sql := 'insert into public.companies (name' ||
               case when public.gmt_table_has_column('companies','code') then ', code' else '' end ||
               case when public.gmt_table_has_column('companies','active') then ', active' else '' end ||
               ') values (' || quote_literal(v_company_name) ||
               case when public.gmt_table_has_column('companies','code') then ', ' || quote_literal(public.gmt_next_code('companies','EMP',3)) else '' end ||
               case when public.gmt_table_has_column('companies','active') then ', true' else '' end ||
               ') returning id';
      execute v_sql into v_company_id;
    elsif public.gmt_table_has_column('companies', 'label') then
      v_sql := 'insert into public.companies (label' ||
               case when public.gmt_table_has_column('companies','code') then ', code' else '' end ||
               case when public.gmt_table_has_column('companies','active') then ', active' else '' end ||
               ') values (' || quote_literal(v_company_name) ||
               case when public.gmt_table_has_column('companies','code') then ', ' || quote_literal(public.gmt_next_code('companies','EMP',3)) else '' end ||
...
begin
  ...
  -- departments/work_centers suelen exigir company_id NOT NULL.
  if public.gmt_table_has_column(v_table, 'company_id') then
    v_company_id := public.gmt_resolve_company_id(v_company_id, v_company_name);
    if v_company_id is null then
      raise exception 'No se pudo resolver company_id para %', coalesce(v_company_name, 'Grupo Marport');
    end if;
  end if;
  ...
  if v_id is not null then
    if public.gmt_table_has_column(v_table, 'name') then
      v_set := array_append(v_set, format('name = %L', coalesce(v_name, v_label, v_role)));
    end if;
    if public.gmt_table_has_column(v_table, 'label') then
      v_set := array_append(v_set, format('label = %L', coalesce(v_label, v_name, v_role)));
    end if;
    if public.gmt_table_has_column(v_table, 'role') then
      v_set := array_append(v_set, format('role = %L', coalesce(v_role, public.gmt_slugify(coalesce(v_label, v_name)))));
    end if;
    if public.gmt_table_has_column(v_table, 'company_id') then
      v_set := array_append(v_set, format('company_id = %L::uuid', v_company_id));
    end if;
    if public.gmt_table_has_column(v_table, 'company_name') then
      v_set := array_append(v_set, format('company_name = %L', coalesce(v_company_name, 'Grupo Marport')));
    end if;
    if public.gmt_table_has_column(v_table, 'description') then
      v_set := array_append(v_set, format('description = %L', nullif(trim(p_payload->>'description'), '')));
    end if;
    if public.gmt_table_has_column(v_table, 'active') then
      v_set := array_append(v_set, format('active = %L::boolean', v_active));
    end if;
    ...
    v_sql := format('update public.%I set %s where id = %L::uuid returning to_jsonb(%I.*)', v_table, array_to_string(v_set, ', '), v_id, v_table);
  ...
  if public.gmt_table_has_column(v_table, 'code') then
    ...
    v_vals := array_append(v_vals, format('%L', v_code));
  end if;
  if public.gmt_table_has_column(v_table, 'name') then
    v_cols := array_append(v_cols, 'name');
    v_vals := array_append(v_vals, format('%L', coalesce(v_name, v_label, v_role)));
  end if;
  if public.gmt_table_has_column(v_table, 'label') then
    v_cols := array_append(v_cols, 'label');
    v_vals := array_append(v_vals, format('%L', coalesce(v_label, v_name, v_role)));
  end if;
  if public.gmt_table_has_column(v_table, 'role') then
    v_cols := array_append(v_cols, 'role');
    v_vals := array_append(v_vals, format('%L', coalesce(v_role, public.gmt_slugify(coalesce(v_label, v_name)))));
  end if;
  if public.gmt_table_has_column(v_table, 'company_id') then
    v_cols := array_append(v_cols, 'company_id');
    v_vals := array_append(v_vals, format('%L::uuid', v_company_id));
  end if;
  if public.gmt_table_has_column(v_table, 'company_name') then
    v_cols := array_append(v_cols, 'company_name');
    v_vals := array_append(v_vals, format('%L', coalesce(v_company_name, 'Grupo Marport')));
  end if;
  if public.gmt_table_has_column(v_table, 'description') then
    v_cols := array_append(v_cols, 'description');
    v_vals := array_append(v_vals, format('%L', nullif(trim(p_payload->>'description'), '')));
  end if;
  if public.gmt_table_has_column(v_table, 'active') then
    v_cols := array_append(v_cols, 'active');
    v_vals := array_append(v_vals, format('%L::boolean', v_active));
  end if;
  ...
  v_sql := format('insert into public.%I (%s) values (%s) returning to_jsonb(%I.*)', v_table, array_to_string(v_cols, ', '), array_to_string(v_vals, ', '), v_table);
  execute v_sql into v_result;
  return v_result;
end;
$$;
...
grant execute on function public.gmt_resolve_company_id(uuid,text) to authenticated;
